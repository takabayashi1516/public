...
//	uartInitType.BaudRate = 57600;
	...
	m_pobjAiLevelSens = new CAiLevelSens(this, ADC1, ADC_CHANNEL_12, 500, 100);
...
void CMain::postDataToEdge()
{
	...
	int rc = getBme280()->readInfo(&data);
#if 0
	::printf("l(%4d): %s: T=%lu[deg-C], P=%lu[hPa], H=%lu[%%]\n",
			__LINE__, __PRETTY_FUNCTION__, data.temp, data.press, data.hum);
#endif
	makeDataToEdge(GET_IDX_RESULT, rc);
	makeDataToEdge(GET_IDX_TEMPERETURE, data.temp);
	makeDataToEdge(GET_IDX_PRESSURE, data.press);
	makeDataToEdge(GET_IDX_HUMIDITY, data.hum);
	...
	getSpi(3)->pushSlaveBuffer(reinterpret_cast<uint8_t *>(&res));
	getSpi(3)->getSlaveBuffer()->update();

#if 1
	getSpi(1)->transmitReceive(CSpiBus::EChannel0,
				(uint8_t *)&res, (uint8_t *)&res, sizeof(command_t), false);
	getSpi(2)->transmitReceive(CSpiBus::EChannel0,
				(uint8_t *)&res, (uint8_t *)&res, sizeof(command_t), false);
#endif
...
int CMain::CExecInputSwitch::doExec(const void *a_lpParam, const int a_cnLength)
{
	...
	if (GPIO_PIN_13 == sw) {
		...
		::setPwmDuty(TIM_CHANNEL_2, 0);
		m_pobjOwner->setTimeout(DEFAULT_MAINTASK_TIMEOUT);
		GPIO_PinState sts = ::HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);
