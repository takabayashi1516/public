...
{
	CUart::interruptHandler(CUart::getInstance(CUart::EChannel6));
}
			...
			m_pobjRingBuffer(NULL), m_pobjCmpltEvent(NULL), m_pobjTx(NULL)
{
	...
	} else if (getDevice() == USART6) {
		m_pobjDriverList[EChannel6] = this;
	} else {
		return;
	}
...
void CUart::rxHandler()
{
	...
	for (; ((dev->SR) & USART_SR_RXNE);) {
		uint16_t tmp = (dev->DR) & 0x01ffu;
...
int CAnalogInput::onInitialize()
{
	...
	*/
	m_objAdcHandleTypeDef.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV4;
	m_objAdcHandleTypeDef.Init.Resolution = ADC_RESOLUTION_12B;
	m_objAdcHandleTypeDef.Init.ScanConvMode = DISABLE;
	m_objAdcHandleTypeDef.Init.ContinuousConvMode = ENABLE;
