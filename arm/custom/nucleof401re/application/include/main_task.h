...
public:
	...
	///
	CBme280 *getBme280() const;
	///
	CZigBee *getZigBee() const;
	///
	CAiIlluminance *getAiIlluminance() const {
		return m_pobjAiIlluminance;
	}

	///
	void setStatus(uint32_t a_wIndex, uint32_t a_wData) {
		m_wStatus[a_wIndex] = a_wData;
	}
	///
	...
	///
	void makeResponse(command_t *a_pobjResponse, command_t *a_pobjCommand, bool a_bAuto = false);

	///
	void sendCommand(command_t *a_pobjCommand, EInterface a_nInterface);

	///
	void setPwmDuty(uint32_t a_unChannel, uint32_t a_unDuty, int a_nPriority = 0);
	///
	void restorePwmDuty(uint32_t a_unChannel, int a_nPriority = 0);
...
private:
	...
	///
	uint32_t getStatus(uint32_t a_wIndex) const {
		return m_wStatus[a_wIndex];
	}
...
private:
	...
	///
	CAiIlluminance	*m_pobjAiIlluminance;
	///
	CObserver		m_objObserver;
	///
	ECommStatus		m_nCommStatus;
	///
	uint32_t		m_wStatus[CMD_VAL_IDX_MAX];

	///
	uint32_t		m_wPwmValue[CMD_VAL_IDX_PWM_BITS];
	///
	int				m_nPwmAplPriority[CMD_VAL_IDX_PWM_BITS];
 ...
 *
 */
class CAiIlluminance : public CAnalogInput {
public:
	///
	CAiIlluminance(CMain *a_objOwner, const ADC_TypeDef *a_cpobjAdc,
