...
int dataAvailable;
uint8_t deviceName[]="STM32ToTangle";
...
uint8_t VALUE1[]="{\"Name\":\"Environment\"}";
...
uint8_t VALUE2[]="{\"Microphone\":\"+0000\"}";
...
uint8_t VALUE_PRESS[]="{\"Press\":\"+000.0\"}";
...
uint8_t NAME_INERTIAL_VALUE[]="{\"Name\":\"Accelerometer\"}";
...
uint8_t ACCX_INERTIAL_VALUE[]="{\"AxisX\":\"+0000\"}";
...
uint8_t ACCY_INERTIAL_VALUE[]="{\"AxisY\":\"+0000\"}";
...
uint8_t ACCZ_INERTIAL_VALUE[]="{\"AxisZ\":\"+0000\"}";

uint8_t X_VALUE[]="{\"AxisX\":\"+0000\"}";
uint8_t Y_VALUE[]="{\"AxisY\":\"+0000\"}";
uint8_t Z_VALUE[]="{\"AxisZ\":\"+0000\"}";
uint8_t MIC_VALUE[]="{\"Microphone\":\"+0000\"}";
...
uint8_t NAME_MAGNETIC_VALUE[]="{\"Name\":\"Magnetometer\"}";
...
uint8_t NAME_GYROSCOPE_VALUE[]="{\"Name\":\"Gyroscope\"}";
...
uint8_t NAME_TOF_VALUE[]="{\"Name\":\"TimeOfFlight\"}";
...
uint8_t TOF_VALUE[]="{\"Distance\":\"+0000\"}";
...
int fetchBleEvent(uint8_t *container, int size){
  ...
  CMain::getSingleton()->getSpi(3)->transmitReceive(CSpiBus::EChannel0, master_header, slave_header, 5);
  ...
  if(dataSize>0){
	    //let's fill the get the bytes availables and insert them into the container variable
  		...
  		CMain::getSingleton()->getSpi(3)->transmitReceive(CSpiBus::EChannel0, (uint8_t *)&dummy, container + i, 1);
...
void sendCommand(uint8_t *command,int size){
	...
	if(bufferSize>=size){
		...
		CMain::getSingleton()->getSpi(3)->transmit(CSpiBus::EChannel0, command, size);
		result=0;
	}else{
		result=-1;
	}
