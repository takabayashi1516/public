...
#define tick_to_ms(t)  ((((t) * 1000) / (1000000 / configTICK_RATE_HZ)))
...
bool CSemaphore::wait(uint32_t a_unTimeout)
{
	...
	if (!CContext::inInterrupt()) {
		result = ::xSemaphoreTake(reinterpret_cast<QueueHandle_t>(m_hResource), tick_to_ms(a_unTimeout));
	} else {
		result = ::xSemaphoreTakeFromISR(reinterpret_cast<QueueHandle_t>(m_hResource), NULL);
	}
...
CThreadFrameBase::CResponseMessage::~CResponseMessage()
{
	::vQueueDelete(reinterpret_cast<QueueHandle_t>(getXferHandle(EXferRead)));
}
...
int	CThreadFrameBase::CResponseMessage::reply(void *a_lpParam)
{
	int nResult = 0;
	if (!CContext::inInterrupt()) {
		nResult = ::xQueueSend(reinterpret_cast<QueueHandle_t>(getXferHandle(EXferWrite)), &a_lpParam, portMAX_DELAY);
	} else {
		nResult = ::xQueueSendFromISR(reinterpret_cast<QueueHandle_t>(getXferHandle(EXferWrite)), &a_lpParam, NULL);
	}
	return(nResult);
}
...
void	*CThreadFrameBase::CResponseMessage::wait(uint32_t a_unTimeout/* = ~0u*/)
{
	...
	(void) ::xQueueReceive(reinterpret_cast<QueueHandle_t>(getXferHandle(EXferRead)), &pResult, tick_to_ms(a_unTimeout));
	return(pResult);
}
			...
			m_unRecvTimeout(a_unRecvTimeout)
{
	...
	nResult = ::xTaskCreate(mainRoutine, reinterpret_cast<char *>(m_byExtra),
			a_unStagesOfStack, this, a_unPriority, reinterpret_cast<TaskHandle_t *>(&m_hThread));
	if (nResult)	{
	}
...
//		::printf("l(%4d): %s: frame=%p: start req=%d\n", __LINE__, __PRETTY_FUNCTION__, this, nRequest);
		...
		uint32_t a_unTimeout)
{
	...
	nResult = ::xQueueReceive(reinterpret_cast<QueueHandle_t>(h), reinterpret_cast<void *>(&msg), tick_to_ms(a_unTimeout));
	if (nResult != pdTRUE) {
		...
		CThreadFrameBase::CRequestMessageBase *a_lpobjMessage)
{
	int nResult = -1;
	if (!CContext::inInterrupt()) {
		nResult = ::xQueueSend(reinterpret_cast<QueueHandle_t>(m_lpobjThreadFrame->getXferHandle(
	...
	nResult = ::xQueueSendFromISR(reinterpret_cast<QueueHandle_t>(m_lpobjThreadFrame->getXferHandle(
 ...
 *
 */
uint32_t CTripleBuffer::getNumOfBuffer() const
{
...
{
	// PS.EXCM(bit[4]) 0: normal mode, 1: exception mode.
	uint32_t value = 0u;
	__asm__ volatile ("rsr.ps %0" : "=a"(value));
	// ::printf("l(%4d): %s: %08lx\n", __LINE__, __PRETTY_FUNCTION__, value);
	value &= (XCHAL_PS_EXCM_MASK);
	// ::printf("l(%4d): %s: %08lx\n", __LINE__, __PRETTY_FUNCTION__, value);
	return (!!value);
}
